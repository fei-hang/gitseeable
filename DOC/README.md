...
在提交列表中，右键点击任意一条**非当前 HEAD 分支**的提交记录，会弹出上下文菜单，选择 **优选此提交** 即可将此次提交的变更应用到当前分支上（相当于执行 `git cherry-pick` 命令）。

- 如果当前选中的提交是 HEAD 分支的顶端，则优选选项不可用
- 优选成功后，提交历史会自动刷新，当前分支会生成一条新的提交记录
- 如果优选过程中产生冲突，会显示错误信息
...
| **重命名** | 修改选中的本地分支的名称。 |
| **删除** | 删除选中的本地分支。无法删除当前所在的分支。 |
| **推送** | 将选中的分支推送到远程仓库。 |
| **拉取** | 从远程仓库拉取最新数据到选中的分支。 |
